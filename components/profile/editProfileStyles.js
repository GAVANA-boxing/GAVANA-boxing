import { GOLD, RED, RED_DARK, BG, BORDER, RADIUS, redAlpha, goldAlpha, whiteAlpha, blackAlpha, pageBg } from "@/lib/tokens";

const styles = {
  page: {
    minHeight: "100dvh",
    background: pageBg(),
    color: "#fff",
    padding: "calc(20px + env(safe-area-inset-top)) 16px 48px",
  },
  shell: {
    width: "min(100%, 560px)",
    margin: "0 auto",
    display: "grid",
    gap: 22,
  },
  backButton: {
    justifySelf: "start",
    width: 40,
    height: 40,
    border: `1px solid ${whiteAlpha(0.1)}`,
    background: whiteAlpha(0.05),
    color: "#fff",
    borderRadius: 12,
    display: "flex",
    alignItems: "center",
    justifyContent: "center",
    cursor: "pointer",
    padding: 0,
  },
  header: {
    textAlign: "center",
  },
  kicker: {
    margin: 0,
    color: RED,
    fontSize: 10,
    fontWeight: 900,
    letterSpacing: 3,
    textTransform: "uppercase",
  },
  title: {
    margin: "8px 0 0",
    fontSize: 32,
    lineHeight: 1,
    fontWeight: 1000,
    fontFamily: "var(--font-display, 'Anton', sans-serif)",
    textTransform: "uppercase",
    letterSpacing: "-0.02em",
  },
  avatarBlock: {
    display: "grid",
    justifyItems: "center",
    gap: 12,
  },
  avatarButton: {
    position: "relative",
    width: 120,
    height: 120,
    borderRadius: "50%",
    border: `2.5px solid ${goldAlpha(0.6)}`,
    background: RED,
    boxShadow: `0 0 0 6px ${redAlpha(0.14)}, 0 20px 54px ${blackAlpha(0.4)}`,
    overflow: "hidden",
    cursor: "pointer",
    padding: 0,
    display: "flex",
    alignItems: "center",
    justifyContent: "center",
    transition: "box-shadow 0.2s",
  },
  avatarImage: {
    width: "100%",
    height: "100%",
    objectFit: "cover",
    display: "block",
  },
  avatarInitial: {
    color: "#fff",
    fontSize: 44,
    fontWeight: 900,
  },
  avatarOverlay: {
    position: "absolute",
    inset: 0,
    background: blackAlpha(0.52),
    display: "flex",
    alignItems: "center",
    justifyContent: "center",
    transition: "opacity 0.18s",
    pointerEvents: "none",
  },
  photoButton: {
    border: `1px solid ${goldAlpha(0.28)}`,
    background: `${goldAlpha(0.08)}`,
    color: GOLD,
    borderRadius: RADIUS.full,
    padding: "9px 16px",
    fontSize: 13,
    fontWeight: 800,
    cursor: "pointer",
    maxWidth: 260,
    overflow: "hidden",
    textOverflow: "ellipsis",
    whiteSpace: "nowrap",
  },
  form: {
    display: "grid",
    gap: 18,
    padding: "20px 18px",
    borderRadius: 20,
    background: whiteAlpha(0.03),
    border: `1px solid ${whiteAlpha(0.07)}`,
    backdropFilter: "blur(16px)",
    WebkitBackdropFilter: "blur(16px)",
    boxShadow: `0 20px 60px ${blackAlpha(0.34)}`,
  },
  field: {
    display: "grid",
    gap: 8,
  },
  label: {
    color: RED,
    fontSize: 10,
    fontWeight: 900,
    letterSpacing: 2,
    textTransform: "uppercase",
  },
  usernameWrap: {
    position: "relative",
  },
  usernameAt: {
    position: "absolute",
    left: 14,
    top: "50%",
    transform: "translateY(-50%)",
    color: whiteAlpha(0.3),
    fontSize: 15,
    fontWeight: 700,
    pointerEvents: "none",
    zIndex: 1,
  },
  input: {
    width: "100%",
    boxSizing: "border-box",
    border: `1px solid ${whiteAlpha(0.1)}`,
    borderRadius: 12,
    background: blackAlpha(0.9),
    color: "#fff",
    padding: "13px 14px",
    fontSize: 15,
    outline: "none",
    fontFamily: "inherit",
    transition: "border-color 0.15s",
  },
  textarea: {
    width: "100%",
    boxSizing: "border-box",
    border: `1px solid ${whiteAlpha(0.1)}`,
    borderRadius: 12,
    background: blackAlpha(0.9),
    color: "#fff",
    padding: "13px 14px",
    fontSize: 15,
    lineHeight: 1.55,
    resize: "none",
    outline: "none",
    fontFamily: "inherit",
  },
  count: {
    justifySelf: "end",
    fontSize: 11,
    fontWeight: 700,
    transition: "color 0.15s",
  },
  chips: {
    display: "flex",
    flexWrap: "wrap",
    gap: 8,
  },
  chip: {
    padding: "8px 14px",
    borderRadius: RADIUS.full,
    border: `1px solid ${whiteAlpha(0.1)}`,
    background: whiteAlpha(0.04),
    color: whiteAlpha(0.5),
    fontSize: 13,
    fontWeight: 700,
    cursor: "pointer",
    transition: "all 0.15s",
    whiteSpace: "nowrap",
  },
  fieldHint: {
    color: whiteAlpha(0.35),
    fontSize: 11,
  },
  fieldError: {
    color: "#F87171",
    fontSize: 11,
    fontWeight: 700,
  },
  error: {
    color: "#fca5a5",
    background: `${redAlpha(0.12)}`,
    border: `1px solid ${redAlpha(0.3)}`,
    borderRadius: 12,
    padding: "12px 14px",
    fontSize: 13,
    lineHeight: 1.5,
  },
  saveButton: {
    width: "100%",
    border: "none",
    borderRadius: RADIUS.md,
    background: `linear-gradient(145deg, ${RED}, ${RED_DARK})`,
    color: "#fff",
    minHeight: 52,
    fontSize: 15,
    fontWeight: 900,
    boxShadow: `0 10px 32px ${redAlpha(0.32)}, inset 0 1px 0 ${whiteAlpha(0.1)}`,
    transition: "box-shadow 0.3s",
    fontFamily: "inherit",
    cursor: "pointer",
  },
};

export default styles;

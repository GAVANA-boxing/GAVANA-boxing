...
const STEPS = [
  ...
  { id: "upload", icon: "🎬", label: "Анхны Reel", desc: "Эхний training reel upload хийх", check: (d) => (d.reelsCount || 0) > 0, cta: "Reel upload", link: "upload" },
  { id: "ai", icon: "🤖", label: "AI дүн авах", desc: "Challenge хийж AI техник шинжилгээ авах", check: (d) => (d.challengesCount || 0) > 0, cta: "Train хийх", link: "train" },
  { id: "streak", icon: "🔥", label: "3 өдрийн streak", desc: "3 өдөр дараалан training хийх", check: (d) => (d.streakDays || 0) >= 3, cta: "Train хийх", link: "train" },
  { id: "coach", icon: "🎯", label: "Coach олох", desc: "Өөртөө тохирсон coach-той холбогдох", check: (d) => !!(d.coachId || d.hasCoach), cta: "Coach хайх", link: "coach" },
  { id: "sparring", icon: "🥊", label: "Sparring partner", desc: "Тулах хүн олох", check: (d) => !!(d.hasSparringPartner), cta: "Sparring хайх", link: "sparring" },
];
    ...
    <div style={fp.wrap}>
      ...
      <div style={fp.stepList}>
        {STEPS.map((step) => {
          ...
          return (
            <div key={step.id} style={{ ...fp.step, ...(isNext ? fp.stepHighlight : {}), ...(done ? fp.stepDone : {}) }}>
              <div style={{ ...fp.stepIcon, ...(done ? fp.iconDone : isNext ? fp.iconActive : fp.iconIdle) }}>
                {done ? "✓" : step.icon}
              </div>
              <div style={fp.stepBody}>
                <div style={{ ...fp.stepLabel, ...(done ? fp.labelDone : isNext ? fp.labelActive : fp.labelIdle) }}>
                  {step.label}
                </div>
                {!done && <div style={fp.stepDesc}>{step.desc}</div>}
              </div>
              {isNext && !step.soon && step.link && (
                <button type="button" onClick={() => router?.push(`/${locale}/${step.link}`)} style={fp.ctaBtn}>
                  {step.cta} →
                </button>
              )}
              {step.soon && <span style={fp.soonTag}>Soon</span>}
            </div>
          );
        })}
      </div>
...
const fp = {
  ...
  dot: { width: 5, height: 5, borderRadius: "50%", flexShrink: 0, background: GOLD, boxShadow: "0 0 7px #D4AF37, 0 0 14px #D4AF3755" },
  ...
  fill: { height: "100%", borderRadius: 999, background: "linear-gradient(90deg, #C1121F, #D4AF37)", transition: "width 700ms cubic-bezier(0.4,0,0.2,1)" },
  ...
  stepHighlight: { background: `${redAlpha(0.05)}`, borderBottom: `1px solid ${redAlpha(0.08)}` },
  stepDone: { opacity: 0.5 },
  stepIcon: { width: 32, height: 32, borderRadius: 10, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15 },
  iconDone: { background: "rgba(52,211,153,0.1)", border: "1px solid rgba(52,211,153,0.2)", color: "#34D399", fontSize: 12, fontWeight: 900 },
  iconActive: { background: `${redAlpha(0.15)}`, border: `1px solid ${redAlpha(0.38)}`, color: "#fff" },
  iconIdle: { background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", color: "rgba(255,255,255,0.28)" },

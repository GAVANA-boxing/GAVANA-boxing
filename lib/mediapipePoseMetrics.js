...
// ─── Final session score (0–10) ───────────────────────────────────────────────
// Blends boxing intelligence (punch quality, guard recovery, rhythm, confidence)
// with pose metrics (stance, balance, extension) when punch data is available.
// Falls back to pose-only score for sessions with fewer than 3 punches.
export function computeFinalScore(summary) {
  const bi           = summary.boxingIntelligence;
  const totalPunches = summary.punchCount || 0;
  const poseScore    = poseMetricsScore(summary);

  if (bi && totalPunches >= 3) {
    const s      = bi.scores || {};
    const breakdown = summary.punchBreakdown || {};

    // Punch quality: weighted average across detected types
    let punchTotal = 0, punchWeight = 0;
    const addPunch = (qualityScore, count) => {
      if (qualityScore == null) return;
      const w = Math.max(count || 0, 1);
      punchTotal  += qualityScore * w;
      punchWeight += w;
    };
    addPunch(s.jabQuality?.score,   breakdown.jab?.count);
    addPunch(s.crossQuality?.score, breakdown.cross?.count);
    addPunch(s.hookQuality?.score,  breakdown.hook?.count);

    // If all punches were low-confidence (filtered out of breakdown), fall back to pose score
    if (punchWeight === 0) return poseScore;

    const punchQuality = punchTotal / punchWeight;
    const guardScore   = s.guardRecovery?.score ?? 50;
    const rhythmScore  = s.comboRhythm?.score   ?? 50;
    const confScore    = summary.avgConfidencePct ?? 70;
    const poseComp     = poseScore != null ? poseScore * 10 : 50;

    // Weights: punch quality most important, then guard discipline, rhythm, confidence, pose
    const blended = (
      punchQuality * 0.35 +
      guardScore   * 0.20 +
      rhythmScore  * 0.15 +
    ...
    return Math.round(Math.min(10, Math.max(0, blended / 10)) * 10) / 10;
  }

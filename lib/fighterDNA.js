...
// Early-signal read from 1+ sessions — shown before full DNA confirms
export function computePreliminarySignals(sessions) {
  if (!sessions || sessions.length === 0) return null;
  let jab = 0, cross = 0, hook = 0;
  for (const s of sessions) {
    const pb = s.poseMetrics?.punchBreakdown;
    if (!pb) continue;
    jab   += pb.jab?.count   || 0;
    cross += pb.cross?.count || 0;
    hook  += pb.hook?.count  || 0;
  }
  ...
  return {
    aggression:      hookPct >= 35 ? "high" : hookPct >= 20 ? "medium" : "low",
    range:           jabPct  >= 45 ? "long" : hookPct >= 30 ? "close"  : "mid",
    counterInstinct: crossPct >= 35 ? "high" : crossPct >= 22 ? "emerging" : "low",
    volume:          (total / sessions.length) >= 25 ? "high" : (total / sessions.length) >= 12 ? "medium" : "building",
  };
}

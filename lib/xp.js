...
export const RANK_TIERS = [
  {
    key: "rankRookieGloves",
    minXP: 0,
    color: "#9CA3AF",
    gradient: "linear-gradient(90deg,#374151,#9CA3AF)",
    icon: "circle",
    glowColor: null,
    pulse: false,
  },
  {
    ...
    minXP: 500,
    color: "#CD7F32",
    gradient: "linear-gradient(90deg,#78350F,#CD7F32)",
    icon: "shield",
  ...
  },
  {
    ...
    minXP: 1500,
    color: "#B87333",
    gradient: "linear-gradient(90deg,#7C2D12,#B87333)",
    icon: "shield",
    glowColor: "rgba(184,115,51,0.65)",
    pulse: false,
  },
  {
    key: "rankSilverBelt",
    minXP: 3000,
    color: "#C0C0C0",
    gradient: "linear-gradient(90deg,#6B7280,#C0C0C0)",
    icon: "star5",
  ...
  },
  {
    ...
    minXP: 6000,
    color: "#F5C451",
    gradient: "linear-gradient(90deg,#92400E,#F5C451)",
    icon: "star5",
    glowColor: `${goldAlpha(0.75)}`,
    pulse: false,
  },
  {
    key: "rankDiamondBelt",
  ...
  },
  {
    ...
    minXP: 25000,
    color: "#F59E0B",
    gradient: "linear-gradient(90deg,#B45309,#F59E0B)",
    icon: "crown",
  ...
  },
  {
    ...
    minXP: 40000,
    color: "#FF3B30",
    gradient: "linear-gradient(90deg,#7F1D1D,#FF3B30)",
    icon: "crown",
    glowColor: `${redAlpha(1.0)}`,
    pulse: true,
  },
];
...
// Max XP per challenge session — prevents runaway grinding
export const SESSION_CHALLENGE_XP_CAP = 500;

export function calculateChallengeXP(score, rank) {
  const numericScore = Number(score);
  const base = Number.isFinite(numericScore) ? Math.round(numericScore * 35) : 0;
  // S rank correctly awards more than A — previous A > S inversion is fixed
  const bonusByRank = {
    S: 150,
    A: 80,
    B: 40,
  ...
  const raw = base + (bonusByRank[String(rank || "").toUpperCase()] || 0);
  return Math.min(raw, SESSION_CHALLENGE_XP_CAP);
}

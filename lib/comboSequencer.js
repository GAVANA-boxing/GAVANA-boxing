...
function _generateCoaching(buffer, gaps, quality) {
  ...
  if (quality.rhythm >= 85 && meanGap < 380 && quality.overall >= 75) {
    return ["Excellent rhythm"];
  }
  if (quality.overall >= 70 && quality.continuity >= 70 && meanGap < 500) {
    return ["Good flow between punches"];
  }

  // Pattern-specific: jab → cross transition
  if (types[0] === "jab" && types[1] === "cross" && gaps[0] > 550) {
  ...
  if (gaps.length >= 2) {
    ...
    if (lastGap > meanGap * 1.65 && lastGap > 480) {

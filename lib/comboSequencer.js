...
// ── Timing ────────────────────────────────────────────────────────────────────
const COMBO_BUFFER_MS   = 200;   // grace window added to recoilMs (was 150)
const COMBO_MIN_GATE_MS = 500;   // floor: fast punches still combo at 10fps (~100ms/frame jitter)
...
// ── Quality thresholds ────────────────────────────────────────────────────────
const FEINT_CONF_THRESH = 0.35;  // was 0.40 — snap bypass punches legitimately score ~0.38
const COMBO_MIN_CONF    = 0.28;  // skip punches too uncertain to be real — noise filter
const FRAME_JITTER_MS   = 80;    // expected detection jitter at 10fps — subtracted from rhythm calc
...
function _scoreQuality(buffer, gaps) {
  ...
  // Rhythm: subtract expected 10fps detection jitter before computing CV.
  // At 10fps a perfectly timed punch still has ~80ms timing noise — don't penalise that.
  const adjustedStdDev = Math.max(0, stdDev - FRAME_JITTER_MS);
  const cv             = meanGap > 0 ? adjustedStdDev / meanGap : 0;
  ...
  const contScores = buffer.slice(1).map((_, i) => {
    ...
    const gate   = Math.min(Math.max(recoil + COMBO_BUFFER_MS, COMBO_MIN_GATE_MS), COMBO_MAX_GAP_MS);
...
export class ComboSequencer {
  ...
  addPunch(event) {
    // Skip punches that are too uncertain — noise filter for false positives
    if ((event.confidence ?? 1) < COMBO_MIN_CONF) return;

    const last = this._buffer[this._buffer.length - 1];
    if (last) {
      const gap  = event.ts - last.ts;
      const gate = Math.min(Math.max((last.recoilMs ?? NOMINAL_RECOIL_MS) + COMBO_BUFFER_MS, COMBO_MIN_GATE_MS), COMBO_MAX_GAP_MS);

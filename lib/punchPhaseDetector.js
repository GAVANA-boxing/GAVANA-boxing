...
const SHOULDER_MOVE_THRESH = 0.040; // shoulder shift > 4% of shoulderWidth/frame = head/body moving, not punching
...
class _HandTracker {
  ...
  update(landmarks, bodyApproach = false, phoneJerk = false) {
    ...
    this._live.shoulderMoving    = shVelNorm > SHOULDER_MOVE_THRESH;
    ...
    switch (this._phase) {
      case "guard": {
        ...
        const canEnter = angle > ANGLE_EXTEND
          ...
          && !onCooldown
          && !phoneJerk
          && shVelNorm <= SHOULDER_MOVE_THRESH;
        ...
        if (canEnter) {
          ...
          if (shVelNorm > SHOULDER_MOVE_THRESH) {
            this._live.triggerReason = `shoulderMove ${shVelNorm.toFixed(3)} (head/body)`;

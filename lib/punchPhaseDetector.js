...
class _HandTracker {
    ...
    switch (this._phase) {
      case "guard": {
        ...
        const canEnter = angle > ANGLE_EXTEND
          ...
          && angleRising
          && (extDeltaNorm > EXT_DELTA_NORM || this._smoothZRel < Z_FWD_GATE)
          && isCalibrated
        ...
        if (canEnter) {
          ...
          // Near-miss: explain what blocked it (helps calibration)
          if (!isCalibrated) {
  ...
  _recordPunch(shoulderWidth, now = Date.now()) {
    ...
    // At 10fps (100ms/frame), legitimate punches always exceed MIN_EXTEND_MS.

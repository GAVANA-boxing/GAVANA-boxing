...
function _typeScores(peakAngle, relExtMaxX, relExtMaxY, zForwardTotal, distIncrease, shoulderWidth) {
  ...
  if      (lateralRatio > 0.55 && !isForward) { hook     += 2; reasons.push(`lat${Math.round(lateralRatio*100)}+noZ h2`); }
  else if (lateralRatio > 0.55 &&  isForward) { straight += 1; reasons.push(`lat${Math.round(lateralRatio*100)}+zFwd s1`); }
  else if (lateralRatio > 0.42 && !isForward) { hook     += 1; reasons.push(`lat${Math.round(lateralRatio*100)}+noZ h1`); }
  else if (lateralRatio < 0.35)               { straight += 2; reasons.push(`lat${Math.round(lateralRatio*100)} s2`); }

...
const MIN_EXTEND_MS      = 60;    // extending must last ≥60ms before punch records (noise gate)
...
class _HandTracker {
  ...
  update(landmarks, bodyApproach = false) {
...
export class PunchPhaseDetector {
  constructor() {
    ...
    this._bodyApproach = false; // set from previous frame's bilateral z check
  }
  ...
  update(landmarks) {
    // Pass previous frame's bilateral approach flag to each tracker,
    // then recompute for next frame after both trackers have updated their smoothZRel.
    const L = this._left.update(landmarks, this._bodyApproach);
    const R = this._right.update(landmarks, this._bodyApproach);

    // Bilateral z-forward = whole body moving toward camera, not a punch.
    // Both hands' shoulder-relative z decreasing simultaneously → suppress z-gate next frame.

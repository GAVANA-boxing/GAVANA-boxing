...
// Bilateral checks use ASYMMETRY to distinguish real punches from body/phone movement.
// Real punch: one hand clearly dominant (2x+ faster, or much more z-forward).
// Body approach / phone jerk: both hands move similarly — low asymmetry.
const BILATERAL_Z_THRESH   = -0.012; // both hands must be below this to even consider body approach
const BILATERAL_Z_DIFF_MAX = 0.007;  // if |lz - rz| > this, one hand is clearly more forward = real punch
const BILATERAL_VEL_THRESH = 0.050;  // both hands must be above this to consider phone jerk
const BILATERAL_VEL_RATIO  = 2.0;    // if dominant/non-dominant > this, one hand is clearly punching
const TOO_CLOSE_SW         = 0.52;   // shoulderWidth > this = camera too close → z unreliable, suppress z-gate
...
// ── Stability gates ────────────────────────────────────────────────────────────
const ANGLE_MIN_RATE     = 3.0;   // °/frame: angle must be RISING (lowered for full-body punches at distance)
const VEL_HIGH_BYPASS    = 0.110; // normVel above this bypasses angle-rate gate (fast snap)
const EXT_BYPASS_RATE    = 0.085; // extDeltaNorm above this bypasses rate gate (arm clearly extending)
...
const SHOULDER_MOVE_THRESH  = 0.035; // shoulder shift rate triggering body-movement gate
const WRIST_DOMINANCE_RATIO = 1.5;   // wrist must be this much faster than shoulder; if not, shoulder is driving the motion

// ── Other ──────────────────────────────────────────────────────────────────────
const MIN_VIS            = 0.50;  // per-frame visibility floor — matches green/red dot threshold in UI
...
class _HandTracker {
  constructor(hand, sIdx, eIdx, wIdx, shOtherIdx) {
    ...
    this._prevShPos      = null;  // previous shoulder xy — for body/head shift detection
  ...
  update(landmarks, bodyApproach = false, phoneJerk = false) {
    ...
    // Shoulder velocity gate — if the shoulder landmark itself shifts, head/body is moving.
    // Wrist-relative-to-shoulder removes translation, but a moving shoulder origin means
    // the relative signal is polluted. Suppress punch entry when shoulder shifts > threshold.
    ...
    // shoulderBodyMoving: shoulder is shifting AND wrist isn't moving significantly faster.
    // Real punch: normVel >> shVelNorm (arm drives). Body/head move: normVel ≈ shVelNorm.
    const shoulderBodyMoving = shVelNorm > SHOULDER_MOVE_THRESH && normVel <= shVelNorm * WRIST_DOMINANCE_RATIO;
    ...
    switch (this._phase) {
      case "guard": {
        ...
        // Arm must be actively extending: angle rising fast, very fast snap, OR arm clearly moving out
        ...
        //  (c) camera is NOT too close — when shoulderWidth > 0.52, z is too noisy to trust
        ...
        if (canEnter) {
          ...
          if (shoulderBodyMoving) {
            this._live.triggerReason = `shoulderMove ${shVelNorm.toFixed(3)} wrist/sh=${(normVel/Math.max(shVelNorm,0.001)).toFixed(1)} (body)`;
...
export class PunchPhaseDetector {
  ...
  update(landmarks) {
    ...
    // ASYMMETRY check: if one wrist is much more forward than the other, it's a real punch.
    // Real jab: left wrist z drops a lot, right stays near 0.
    // Body approach: both wrists z drop by similar amounts (low |lz - rz|).
    ...
    this._bodyApproach = lz < BILATERAL_Z_THRESH
      ...
      && zDiff < BILATERAL_Z_DIFF_MAX; // symmetric z = body, asymmetric = real punch

    // Bilateral velocity spike = phone jerk (tap, drop, sudden move), not a punch.
    // ASYMMETRY check: real punches have one dominant hand (2x+ faster than the other).
    // Phone jerk / body walk: both hands elevate with similar speeds (low ratio).
    ...
    this._phoneJerk = lv > BILATERAL_VEL_THRESH
      ...
      && velRatio < BILATERAL_VEL_RATIO; // no dominant hand = phone jerk, not a punch

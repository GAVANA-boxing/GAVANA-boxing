...
function _typeScores(peakAngle, relExtMaxX, relExtMaxY, zForwardTotal, distIncrease, shoulderWidth) {
  ...
  else if (peakAngle >= 128) { straight += 2; reasons.push(`a${peakAngle}s2`); }
  else if (peakAngle >= 118) { straight += 1; reasons.push(`a${peakAngle}s1`); }
  else if (peakAngle <  115) {
    // Low 2D angle has two causes: (a) real hook (lateral arc), (b) front-camera straight punch
    // (foreshortening — arm goes toward camera, 2D angle barely changes).
    // Distinguish: front-camera straight has strong z-forward AND some extension.
    // Hook has lateral arc but NOT significant z-forward.
    if (zForwardTotal < -0.025 && extNorm > 0.05) {
      straight += 1; reasons.push(`a${peakAngle}zFwd s1`);
    } else {
      hook     += 3; reasons.push(`a${peakAngle}h3`);
    }
  }
...
class _HandTracker {
  constructor(hand, sIdx, eIdx, wIdx, shOtherIdx) {
    ...
    this._rejectCounts   = { calib: 0, cooldown: 0, unstable: 0, rate: 0, noExt: 0, brief: 0, body: 0, jerk: 0, lowVel: 0 };
  ...
  update(landmarks, bodyApproach = false, phoneJerk = false) {
    ...
    switch (this._phase) {
      case "guard": {
        ...
        // Angle gate: normally requires elbow to open to ANGLE_EXTEND (75°).
        // Front-camera straight punches (jab/cross aimed directly at camera) never open much
        // in 2D due to foreshortening — the arm extends in Z, not X/Y.
        // When z-forward evidence is strong, bypass the 2D angle requirement.
        const angleGateOK = angle > ANGLE_EXTEND || zFwdValid;

        const canEnter = angleGateOK
        ...
        if (canEnter) {
          ...
          if (shoulderBlocked) {
            ...
            this._live.triggerReason = `lowVel nV${normVel.toFixed(3)}<${VEL_MIN_NORM}`;
            this._rejectCounts.lowVel++;
          }
        }
  ...
  _recordPunch(shoulderWidth, now = Date.now()) {
    ...
    // Absolute minimum angle gate: peakAngle < 25° means the arm barely moved from
    // guard — this is noise/jitter (e.g. a3.24° false positive), not a real punch.
    if (this._peakAngle < 25) {
      this._rejectCounts.brief++;
      this._resetPunch();
      return;
    }
    ...
    // Z-forward entry = punch went toward camera (foreshortening). Low 2D angle ≠ lateral arc.
    // Override hook classification when entry context contradicts arc evidence.
    const snapForcedStraight = this._snapBypassUsed && hook > straight;
    const zFwdForcedStraight = this._zFwdEntry && hook > straight && lateralRatio < 0.50;
    const isHook = !snapForcedStraight && !zFwdForcedStraight && hook > straight;
  ...
  getSessionStats() {
    ...
    return {
      rejects: {
        ...
        jerk:     (l.jerk     || 0) + (r.jerk     || 0),
        lowVel:   (l.lowVel   || 0) + (r.lowVel   || 0),

...
export function generateCoachingMessages(sessionSummary, punchEvents = []) {
  ...
  // ── Punch extension fallback ──────────────────────────────────────────────
  // Only show if punches were actually detected — prevents false "under_extended"
  // from average pose analysis when no punches registered at all.
  if (!hasPunches && punchEvents.length > 0 && sessionSummary.punchExtension?.status === "under_extended") {
    improve.push({ type: "improve", message: "Extension incomplete — straighten the arm all the way through the punch" });
  }
  if (punchEvents.length === 0 && sessionSummary.punchCount === 0 && sessionSummary.frameCount >= 20) {
    cautions.push({ type: "caution", message: "No punches registered — try stepping back so full arm is in frame" });
  }
  ...
  if (lowConfPunches > 0 && punchEvents.length > 0) {
    ...
    if (pct >= 60) {
      // Majority low-confidence: suppress type claims, show generic message
      cautions.push({ type: "caution", message: "Punches detected, type uncertain — punch more directly toward camera" });
    } else if (pct >= 40) {
      cautions.push({ type: "caution", message: `${pct}% of punches were low-confidence — step back for better framing` });
    }
  }

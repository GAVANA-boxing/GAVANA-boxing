...
// ── Phase 5: Real Combat System ───────────────────────────────────────────────
// Round-by-round breakdown and AI corner advice.
// Sessions are split into halves ("round 1" / "round 2") since drill durations
// vary (30-120s) and there is no explicit round boundary in the current system.

function _halfStats(events) {
  if (!events.length) return { count: 0, avgQuality: 0, pace: 0, durationMs: 0 };
  const durationMs = events.length > 1 ? events[events.length - 1].ts - events[0].ts : 0;
  const avgQuality = Math.round(
    events.reduce((s, e) => s + (e.confidence ?? 0.5), 0) / events.length * 100
  );
  const pace = durationMs > 0 ? Math.round((events.length / durationMs) * 60000) : 0;
  return { count: events.length, avgQuality, pace, durationMs };
}

export function computeRoundBreakdown(punchEvents) {
  if (punchEvents.length < 6) return null;

  const mid   = Math.floor(punchEvents.length / 2);
  const half1 = punchEvents.slice(0, mid);
  const half2 = punchEvents.slice(mid);

  const r1 = _halfStats(half1);
  const r2 = _halfStats(half2);

  const paceTrend =
    r2.pace > r1.pace * 1.1 ? "accelerating" :
    r2.pace < r1.pace * 0.9 ? "decelerating"  : "steady";

  const qualityTrend =
    r2.avgQuality < r1.avgQuality - 8 ? "fading" :
    r2.avgQuality > r1.avgQuality + 8 ? "improving" : "stable";
...
export function computeCornerAdvice(punchEvents, tactical, defensive, ringIQ, fighterDNA) {
  const advice = [];

  if (!punchEvents.length) return advice;

  const rounds = computeRoundBreakdown(punchEvents);

  // Fatigue / quality fade
  if (rounds?.qualityTrend === "fading") {
    advice.push("Quality dropped in the second half — work on conditioning to hold your mechanics late");
  }

  // Pacing
  if (rounds?.paceTrend === "decelerating") {
    advice.push("Work rate slowed as the session went on — push through the discomfort in the final stretch");
  } else if (rounds?.paceTrend === "accelerating") {
    advice.push("Good — you raised your pace through the session. Channel that for finishing combinations");
  }

  // Tactical
  if (tactical?.singleShotPct > 65) {
    advice.push("Build combinations off your power shots — a single punch gives your opponent time to reset");
  } else if (tactical?.profile === "pressure_builder") {
    advice.push("Your pressure is working — vary the angle of your pressure to avoid being countered");
  }

  // Defensive
  if (defensive?.defensiveStyle === "guard_reliant") {
    advice.push("Add head movement to your defense — slips and bobs reduce incoming damage significantly");
  }

  // Ring IQ
  if ((ringIQ?.panicFlurries?.length ?? 0) > 0) {
    advice.push("Avoid panic bursts — controlled combinations score better and cost less energy");
  }
  if (ringIQ?.overcommitRate >= 25) {
    advice.push("Stay compact — get your hands back after every punch before committing to the next");
  }

  // DNA archetype encouragement
  if (fighterDNA?.archetype && fighterDNA.archetype !== "raw") {
    advice.push(`Your ${fighterDNA.archetypeLabel} tendencies are showing — keep developing that identity`);
  }
...
export function analyzeSession(sessionSummary, punchEvents = []) {
  ...
  const roundBreakdown = computeRoundBreakdown(punchEvents);
  const cornerAdvice   = computeCornerAdvice(punchEvents, tactical, defensive, ringIQ, fighterDNA);

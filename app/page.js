...
const FAQ = [
  ...
  {q:'Эхлэгчид ямар тоног төхөөрөмж хэрэгтэй вэ?', a:'Бороо, бокс бээлий (16oz), хоолой хамгаалагч, толгой хамгаалагч. ~80,000-150,000₮.'},
  ...
  {q:'Бокс яагаад жин буурдаг вэ?', a:'Нэг цагт 600-1000 калори шатаадаг. Кардио + булчингийн дасгал хосолдог.'},
...
export default function Home() {
  ...
  const callAI = async (prompt) => {
    ...
    return data.content?.[0]?.text || 'Алдаа';
  };

  const send = async () => {
    if (!inp.trim() || load) return;
    const newMsgs = [...msgs, {role:'user',content:inp}];
    setMsgs(newMsgs); setInp(''); setLoad(true);
    try {
      const res = await fetch('/api/chat', {
        method:'POST', headers:{'Content-Type':'application/json'},
      ...
      const data = await res.json();
      setMsgs(p => [...p, {role:'assistant',content:data.content?.[0]?.text||'Алдаа'}]);
    } catch(e) { setMsgs(p => [...p, {role:'assistant',content:'Алдаа'}]); }
    setLoad(false);
  };
  ...
  const s = {
    app:{minHeight:'100vh',background:'#080808',color:'#fff',fontFamily:'sans-serif',maxWidth:480,margin:'0 auto'},
    header:{background:'#0f0f0f',borderBottom:'2px solid #E8002D',padding:'14px 16px',textAlign:'center'},
    title:{color:'#E8002D',fontSize:26,fontWeight:900,letterSpacing:6,margin:0},
    sub:{color:'#555',fontSize:10,letterSpacing:4,margin:0},
    nav:{display:'flex',overflowX:'auto',padding:'8px 12px',gap:6,background:'#0a0a0a',borderBottom:'1px solid #1a1a1a'},
    navBtn:(a)=>({flexShrink:0,padding:'6px 12px',background:a?'#E8002D':'transparent',border:`1px solid ${a?'#E8002D':'#222'}`,color:a?'#fff':'#666',borderRadius:20,fontSize:11,cursor:'pointer',whiteSpace:'nowrap'}),
    content:{padding:'16px 14px 80px'},
    card:{background:'#111',border:'1px solid #1f1f1f',borderRadius:14,padding:16,marginBottom:12},
    btn:{padding:'12px 16px',background:'linear-gradient(135deg,#E8002D,#FF6D00)',color:'#fff',border:'none',borderRadius:10,cursor:'pointer',width:'100%',fontSize:16,fontWeight:700,marginTop:8},
  };

  return (
    <div style={s.app}>
      <div style={s.header}>
        <h1 style={s.title}>GAVANA BOXING</h1>
        <p style={s.sub}>AI ДАСГАЛЖУУЛАГЧ</p>
      </div>
      ...
      <div style={s.content}>
        ...
        {/* HOME */}
        {tab===0 && <div>
          <div style={{...s.card,textAlign:'center',background:'linear-gradient(135deg,#1a0000,#111)'}}>
          ...
          {[{icon:'🦋',t:'Амьтдын хөдөлгөөн',d:'Бодит анимэйшн зурагтай',i:1},{icon:'💨',t:'Амьсгалын техник',d:'4 арга + мастер боксчид',i:2},{icon:'💪',t:'Дасгалын хуваарь',d:'AI хуваарь гаргана',i:3},{icon:'🥗',t:'Хоол тэжээл',d:'AI хоол тооцоолно',i:5}].map(c=>(
            <div key={c.i} onClick={()=>setTab(c.i)} style={{...s.card,display:'flex',gap:14,alignItems:'center',cursor:'pointer'}}>
              <div style={{fontSize:32}}>{c.icon}</div>
              <div><div style={{fontWeight:700,marginBottom:2}}>{c.t}</div><div style={{color:'#555',fontSize:12}}>{c.d}</div></div>
            </div>
          ))}
          <div style={{...s.card,background:'linear-gradient(135deg,#0a1200,#111)',border:'1px solid #76FF0333'}}>
        ...
        {tab===1 && <div>
          ...
          {ANIMALS.map((a,i)=>(
            <div key={i} style={{...s.card,border:`1px solid ${a.color}33`}}>
              <div style={{display:'flex',gap:10,alignItems:'center',marginBottom:10}}>
                <span style={{fontSize:28}}>{a.icon}</span>
                <div><div style={{color:a.color,fontWeight:700,fontSize:16}}>{a.name}</div><div style={{color:'#555',fontSize:11}}>Боксч: {a.boxer}</div></div>
              </div>
              <div style={{color:'#888',fontSize:13,marginBottom:10,lineHeight:1.6}}>{a.move}</div>
              <button onClick={()=>setModal(a)} style={{...s.btn,background:`${a.color}22`,color:a.color,border:`1px solid ${a.color}44`,marginTop:0}}>🥊 {a.boxer} — Дэлгэрэнгүй</button>
            </div>
          ))}
        </div>}
        ...
        {tab===2 && <div>
          ...
          <div style={{color:'#555',fontSize:12,marginBottom:16}}>Нэр дарахад боксчийн тухай мэдэх</div>
          {BREATHING.map((b,i)=>(
            <div key={i} style={{...s.card,border:`1px solid ${b.color}33`}}>
              ...
              <button onClick={()=>setModal(b)} style={{...s.btn,background:`${b.color}22`,color:b.color,border:`1px solid ${b.color}44`,marginTop:0}}>🥊 {b.boxer} — Дэлгэрэнгүй</button>
            </div>
          ))}
        </div>}
        ...
        {tab===3 && <div>
          ...
          <div style={s.card}>
            <div style={{color:'#888',fontSize:13,marginBottom:8}}>Боксын 7 хоногийн хуваарь авах</div>
            <button onClick={async()=>{
              setLoad(true);
              const txt = await callAI('Боксчдод зориулсан 7 хоногийн дасгалын хуваарь гарга. Өдөр бүр: дасгалын нэр, хугацаа, давталт. Монгол хэлээр, товч.');
              setMsgs([{role:'assistant',content:txt}]); setTab(6); setLoad(false);
            }} style={s.btn}>{load?'Боловсруулж байна...':'🥊 AI ХУВААРЬ ГАРГАХ'}</button>
          </div>
          <div style={s.card}>
          ...
          <div style={s.card}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:6,marginBottom:12}}>
              {['Жин бууруулах','Жин хадгалах','Булчин нэмэх'].map((g,i)=>(
                <button key={i} onClick={()=>setNutGoal(i)} style={{padding:'10px',background:nutGoal===i?'#E8002D22':'#1a1a1a',border:`1px solid ${nutGoal===i?'#E8002D':'#222'}`,color:nutGoal===i?'#E8002D':'#666',borderRadius:8,cursor:'pointer',fontSize:13,textAlign:'left'}}>
                  {nutGoal===i?'✓ ':''}{g}
                </button>
              ))}
            </div>
            <button onClick={genNut} style={s.btn}>{nutLoad?'Тооцоолж байна...':'🥗 AI ХООЛ ТООЦООЛОХ'}</button>
          </div>
          {nutPlan&&<div style={{...s.card,border:'1px solid #76FF0333'}}>
            ...
            <div style={{flex:1,overflowY:'auto',display:'flex',flexDirection:'column',gap:10,marginBottom:12}}>
              {msgs.map((m,i)=>(
                <div key={i} style={{display:'flex',justifyContent:m.role==='user'?'flex-end':'flex-start'}}>
                  <div style={{maxWidth:'82%',padding:'10px 14px',borderRadius:14,background:m.role==='user'?'#E8002D':'#222',fontSize:14,lineHeight:1.6}}>{m.content}</div>
                </div>
              ))}
        ...
        {tab===6 && <div>
          ...
          {FAQ.map((f,i)=>(
            <div key={i} style={{...s.card,padding:0,overflow:'hidden',border:`1px solid ${openFaq===i?'#E8002D':'#1f1f1f'}`}}>
      ...
      {modal&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.9)',zIndex:200,display:'flex',alignItems:'flex-end'}} onClick={()=>setModal(null)}>
          <div style={{width:'100%',maxWidth:480,margin:'0 auto',background:'#111',border:'1px solid #222',borderRadius:'20px 20px 0 0',padding:24}} onClick={e=>e.stopPropagation()}>
            ...
            <button onClick={()=>setModal(null)} style={{...s.btn,background:'#1a1a1a',color:'#666',marginTop:16}}>Хаах</button>
          </div>
        </div>
      )}
    </div>

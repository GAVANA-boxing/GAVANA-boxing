  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Tribe count — query users with same archetype (runs after DNA is stable)
        if (!dna.building && dna.archetypeKey) {
          const tribeSnap = await getDocs(query(
            collection(db, "users"),
            where("fighterDNA.archetypeKey", "==", dna.archetypeKey)
          ));
          if (active) setTribeCount(tribeSnap.size);
        }
      } catch { /* silent */ }
    })();
    return () => { active = false; };

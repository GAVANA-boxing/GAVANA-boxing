...
export default function DiscoverPage() {
  ...
  const desktopSidebar = (
    <aside style={{
      ...
      <p style={{ fontSize: 9, fontWeight: 900, letterSpacing: 3.5, textTransform: "uppercase", color: "rgba(255,59,48,0.7)", padding: "0 16px 12px", margin: 0 }}>
        ТУЛААНЫ МЭДЛЭГ
      </p>
      ...
      <div style={{ padding: "0 16px 24px" }}>
        <div style={s.fighterStudyHeader}>
          ...
          <button type="button" style={s.fighterStudySeeAll} onClick={() => router.push(`/${locale}/fighters`)}>
            {t("fighterStudySeeAll")} ›
          </button>
        </div>
        <div style={s.fighterStudyScroll}>
          {FIGHTERS.map((f) => (
            <FighterStudyCard key={f.id} fighter={f} onClick={() => router.push(`/${locale}/fighters/${f.id}`)} />
          ))}
        </div>
      </div>
  ...
  const mainContent = (
    <div style={s.content}>
      ...
      {/* ════════════════════════════════════════
          FIGHTER STUDY — only on mobile (desktop shows in sidebar)
      ════════════════════════════════════════ */}
      {!isDesktop && (
      ...
      {/* ════════════════════════════════════════
          HUB 3 — ⚔ CHALLENGES — only on mobile (desktop shows in sidebar)
      ════════════════════════════════════════ */}
      {!isDesktop && (
        <HubCard
          emoji="⚔️"
  ...
  if (isDesktop) {
    return (
      <div style={{ height: "100dvh", overflow: "hidden", display: "flex", flexDirection: "column", background: "#0B0B0C" }} className="page-enter">
